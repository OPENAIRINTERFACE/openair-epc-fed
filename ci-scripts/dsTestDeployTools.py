...
class deployForDsTester():
    ...
    def deployCassandra(self):
        ...
        while doLoop:
            try:
                res = subprocess.check_output('docker exec cicd-cassandra /bin/bash -c "nodetool status" 2>/dev/null', shell=True, universal_newlines=True)
                ...
                time.sleep(1)
                pass
        subprocess_run_w_echo('docker exec cicd-cassandra /bin/bash -c "nodetool status" 2>&1 | tee archives/cassandra_status.log')
        subprocess_run_w_echo('docker cp component/oai-hss/src/hss_rel14/db/oai_db.cql cicd-cassandra:/home')
        time.sleep(2)
        doLoop = True
        while doLoop:
            try:
                res = subprocess.check_output('docker exec cicd-cassandra /bin/bash -c "cqlsh --file /home/oai_db.cql ' + CICD_CASS_IP_ADDR + '" 2>/dev/null', shell=True, universal_newlines=True)
    ...
    def deployHSS(self):
        ...
        subprocess_run_w_echo('docker exec cicd-oai-hss /bin/bash -c "cd /openair-hss/scripts && chmod 777 hss-cfg.sh && ./hss-cfg.sh" > archives/hss_config.log 2>&1')
    ...
    def deployMME(self):
        ...
        subprocess_run_w_echo('docker exec cicd-oai-mme /bin/bash -c "cd /openair-mme/scripts && chmod 777 mme-cfg.sh && ./mme-cfg.sh" > archives/mme_config.log 2>&1')
    ...
    def deploySPGWC(self):
        ...
        res2 = ''
        try:
            res2 = subprocess.check_output('docker image inspect oai-spgwu-tiny:' + self.peerTag, shell=True, universal_newlines=True)
        except:
            sys.exit(-1)
        # Does one of the tags require "multi-spgwu" support?
        spgwc_multi = re.search('"support-multi-sgwu-instances": "true"', str(res))
        spgwu_multi = re.search('"support-multi-sgwu-instances": "true"', str(res2))
        if (spgwc_multi is not None) or (spgwu_multi is not None):
            if (spgwu_multi is not None) and (spgwc_multi is None):
                self.tag = 'multi-spgwu'
            subprocess_run_w_echo('python3 ci-scripts/generate_spgwc_config_script.py --kind=SPGW-C --s11c=eth0 --sxc=eth0 --prefix=/openair-spgwc/etc --dns1=192.168.18.129 --dns2=8.8.8.8 --apn_list="apn1.carrier.com apn2.carrier.com" --pdn_list="12.0.0.0/24 12.1.0.0/24" --env_list --from_docker_file')
            subprocess_run_w_echo('docker run --privileged --name cicd-oai-spgwc --network cicd-oai-public-net --ip ' + CICD_SPGWC_PUBLIC_ADDR + ' --env-file ./spgw_c_env.list -d --entrypoint "/bin/bash" oai-spgwc:' + self.tag + ' -c "sleep infinity"')
            subprocess_run_w_echo('docker exec cicd-oai-spgwc /bin/bash -c "./bin/entrypoint.sh" > archives/spgwc_config.log')
            subprocess_run_w_echo('docker exec cicd-oai-spgwc /bin/bash -c "grep -c @ etc/spgw_c.json || true" > archives/spgwc_config.log')
            subprocess_run_w_echo('docker exec cicd-oai-spgwc /bin/bash -c "cd etc && ln -s spgw_c.json spgw_c.conf"')
            return
    ...
    def deploySPGWU(self):
        ...
        res2 = ''
        try:
            res2 = subprocess.check_output('docker image inspect oai-spgwc:' + self.peerTag, shell=True, universal_newlines=True)
        except:
            sys.exit(-1)
        # Does one of the tags require "multi-spgwu" support?
        spgwc_multi = re.search('"support-multi-sgwu-instances": "true"', str(res2))
        spgwu_multi = re.search('"support-multi-sgwu-instances": "true"', str(res))
        if (spgwc_multi is not None) or (spgwu_multi is not None):
            if (spgwc_multi is not None) and (spgwu_multi is None):
                self.tag = 'multi-spgwu'
            addFQDNOption = ' --addFQDN'
        else:
            addFQDNOption = ''
        ...
        subprocess_run_w_echo('python3 ci-scripts/generate_spgwu-tiny_config_script.py --kind=SPGW-U --sxc_ip_addr=' + CICD_SPGWC_PUBLIC_ADDR + ' --sxu=eth0 --s1u=eth0 --sgi=eth0 --pdn_list="12.0.0.0/24 12.1.0.0/24" --prefix=/openair-spgwu-tiny/etc --from_docker_file' + addFQDNOption)
...
def Usage():
    ...
    print('  --peerTag=[CUPS Peer Image Tag in registry]')
    ...
    print('python3 dsTestDeployTools.py --action=DeploySPGWC --tag=[tag] --peerTag=[spgwuTag]')
    print('python3 dsTestDeployTools.py --action=DeploySPGWU --tag=[tag] --peerTag=[spgwcTag]')
...
while len(argvs) > 1:
    ...
    elif re.match('^\-\-peerTag=(.+)$', myArgv, re.IGNORECASE):
        matchReg = re.match('^\-\-peerTag=(.+)$', myArgv, re.IGNORECASE)
        DFDT.peerTag = matchReg.group(1)
